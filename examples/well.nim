        ...
        while not window.exit.bool and not window.timerError.bool:
            ...
            while scale > 0:           
                for i in 1..3:
                    ...
                    engine.drawRhombus(0, 0, (scale - shift).float32, (scale - shift).float32, true, Color = shade)
                    engine.renderRotate(-angle * 2 + i.float32 * 2)
                    engine.drawBox(0, 0, (scale - shift).float32, (scale - shift).float32, true, Color = shade)
                    engine.renderRotate(angle-i.float32)                
                scale = scale div 2
            engine.renderReset()
